...
async function requestDesignWorks(section: DesignSection): Promise<DesignPage> {
...
async function getFirstDesignPage(
  ...
  const request = requestDesignWorks(section).finally(() => {
...
export async function loadDesignWorks(options: LoadDesignsOptions = {}): Promise<DesignPage> {
  ...
  const page = await getFirstDesignPage(section, options.forceRefresh === true)
